...
[{if $oOrder }]
    ...
    <div class="alert [{ $alertClass }]">
        ...
        <span style='float: right'>
            [{if $oOrder->oxorder__klorderid->value }]
                [{assign var=klorderid value=$oOrder->oxorder__klorderid->value}]
            [{else}]
                [{assign var=klorderid value=" - "}]
            [{/if}]
            [{if $sKlarnaRef eq ''}]
                [{assign var=sKlarnaRef value=" - "}]
            [{/if}]
            <strong>Klarna order ID:</strong> <i>[{$klorderid}]</i> <strong>Klarna reference:</strong> <i>[{$sKlarnaRef}]</i>
        </span>
