    ...
    public function __construct(oxBasket $oBasket, oxUser $oUser)
    {
        ...
        if ($this->isValidationEnabled()) {
            $this->_aOrderData["merchant_urls"]["validation"] =
                $sSSLShopURL . "?cl=klarna_validate&s=$sessionId";
        }
...
//                    "id"          => 'SRV_DELIVERY',
        ...
        if (empty($shippingOptions)) {
            ...
            $oEx = new KlarnaConfigException(sprintf(
    ...
    protected function getAdditionalCheckbox()
    {
        ...
        $type = $this->_oUser->kl_getType();
        if ($type === klarna_oxuser::LOGGED_IN || $type === klarna_oxuser::REGISTERED) {
    ...
    {
        $oKlarnaPassThrough = new Klarna_Pass_Through();
