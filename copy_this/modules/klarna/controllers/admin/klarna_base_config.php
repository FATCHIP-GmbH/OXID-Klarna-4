...
class klarna_base_config extends Shop_Config
{
    ...
    {
        $langTag = oxRegistry::getLang()->getLanguageAbbr($this->getViewDataElement('adminlang'));
        $oxModuleList = oxNew('oxModuleList');
        $versionList = $oxModuleList->getModuleVersions();
        $version = '4.0.0';
        if(key_exists('klarna', $versionList))
        {
            $version = $versionList['klarna'];
        }
